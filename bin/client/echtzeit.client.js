...
echtzeit.Transport = echtzeit.extend(echtzeit.Class({
  ...
  retry: function(message, timeout) {
    ...
    return function() {
      if (called) return;
      called = true;
      echtzeit.ENV.setTimeout(function() { self.request(message, timeout) }, retry);
    };
  }
...
echtzeit.extend(echtzeit.Transport.prototype, echtzeit.Publisher);
...
echtzeit.Transport.WebSocket = echtzeit.extend(echtzeit.Class(echtzeit.Transport, {
  ...
  request: function(messages, timeout) {
    ...
    this.callback(function(socket) { socket.send(echtzeit.toJSON(messages)) });
    this.connect();
  },
  ...
  connect: function() {
    ...
    this._socket = new ws(echtzeit.Transport.WebSocket.getSocketUrl(this.endpoint));
    var self = this;
    ...
    this._socket.onclose = this._socket.onerror = function() {
      ...
      echtzeit.ENV.setTimeout(function() { self.connect() }, retry);
      self.trigger('down');
    };
  },
  ...
  resend: function() {
    ...
    var messages = echtzeit.map(this._messages, function(id, msg) { return msg });
    this.request(messages);
  }
}), {
  getSocketUrl: function(endpoint) {
    if (echtzeit.URI) endpoint = echtzeit.URI.parse(endpoint).toURL();
...
echtzeit.Transport.XHR = echtzeit.extend(echtzeit.Class(echtzeit.Transport, {
  request: function(message, timeout) {
    var retry = this.retry(message, timeout),
        ...
        self  = this,
        xhr   = echtzeit.ENV.ActiveXObject
...
echtzeit.Transport.JSONP = echtzeit.extend(echtzeit.Class(echtzeit.Transport, {
  ...
  request: function(messages, timeout) {
    var params       = {message: echtzeit.toJSON(messages)},
